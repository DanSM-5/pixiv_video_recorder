...
ZipImagePlayer.prototype = {
    ...
    _loadNextChunk: function() {
        ...
        this._load(off, len, function() {
            if (off == this._pHead) {
                ...
                var loadingProgress = this._pHead / this._len;
                $(this).triggerHandler("loadProgress",
                                       [loadingProgress]);
                this._debugLog(`Loading: ${loadingProgress}`);
                if (`${loadingProgress}` === "1") {
                    $(this).triggerHandler("loadingComplete", { progress: loadingProgress });
                }
                if (!this._loadTimer) {
                    this._loadNextFrame();
                }
    ...
    _displayFrame: function() {
        ...
        if (this._isRecording) {
            this?.captureFrame(this.op.canvas, meta.delay);
        }
    ...
    _nextFrame: function(frame) {
        if (this._frame >= (this._frameCount - 1)) {
            this._cicle += 1;
            var cicle = this._cicle;
            $(this).triggerHandler("ciclecomplete", { cicle });
            if (this._isReady && !this.op.loop && !this._isRecording) {
                this.pause();
                return;
            } else if (!this._isReady && this._isPreparing) {
                this._frame = 0;
                this._debugLog("Restarting frame");
            } else if (this._isReady && this._isRecording) {
                this._frame = 0;
                this._debugLog("Restarting frame");
            }
        } else {
    ...
    recordGif: async function () {
        ...
        this._isRecording = true;
        this.op.debug = true;
        this._debugLog("Start recording...");
        const _this = this;
        const lastPromise = {};
        const recordGifPromise = new Promise((resolve, reject) => {
            lastPromise.reject = reject;
            lastPromise.resolve = resolve;
            const onComplete = evt => {
                _this.pause();
                _this._isRecording = false;
                _this.op.debug = false;
                $(_this).off("ciclecomplete", onComplete);
                _this._reset();
                resolve();
            };
            $(_this).on("ciclecomplete", onComplete);
            _this.play(); // start for real
        });
        lastPromise.promise = recordGifPromise;
        this.lastPromise = lastPromise;
        return recordGifPromise;
    },
    isReady: function () {
    ...
    prepare: async function () {
        ...
        const preparePromise = new Promise((resolve, reject) => {
            ...
            _this._debugLog("Player not ready. Starting loading...");
            const loadingComplete = () => {
                $(_this).off("loadingComplete", loadingComplete);
                $(_this).triggerHandler("playerready");
                _this._isReady = true;
                _this._isPreparing = false;
                _this._debugLog("On loadingComplete");
                _this._reset();
                resolve();
            };
            $(_this).on("loadingComplete", loadingComplete);
            _this.play();
            _this._timeOut(reject, _this._frameCount * 10);
        });
        lastPromise.promise = preparePromise;
        this.lastPromise = lastPromise;
    ...
    _reset: function () {
        this._isPreparing = false;
        this._isRecording = false;
        this.resetCaptureFrame();
        this.resetCancelCheck();
        this.pause();
        this.op.debug = false;
        this.lastPromise = null;
    },
    _timeOut: function (reject, time = 15) {
        setTimeout(() => reject("Time out"), time * 1000);
    },
    captureFrame: function (canvas, time) {
        this?._captureFrameCallback?.(canvas, time);
    },
    setCaptureFrame: function (cb) {
        this._captureFrameCallback = cb;
    },
    resetCaptureFrame: function () {
        this._captureFrameCallback = () => {};
    },
    setCancelCheck: function (predicate, cb) {
    ...
    cancelCheck: function () {
        ...
        this._frame = 0;
        this.pause();
        this?.lastPromise?.reject?.("Recording was canceled!");
        this._reset();
        return true;
    },
    getCanvas: function () {
        return this.op.canvas;
    },
...
window.ZipImagePlayer = ZipImagePlayer;
})();