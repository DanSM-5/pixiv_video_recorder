...
ZipImagePlayer.prototype = {
    ...
    _nextFrame: function(frame) {
        if (this._frame >= (this._frameCount - 1)) {
            ...
            // prepare the player
            if (!(this._cicle <= this._minCicle) && !this._isReady) {
                // run twice before start recording to ensure all images are ready
                this._isReady = true;
                this._debugLog("Player is ready!");
                this.pause();
                $(this).triggerHandler("playerready");
                return;
            } else if (this._isReady && this._isRecording) {
                // Continue loop
                // Recording is managing the cicles
            } else if (this._isReady && !this.op.loop && !this._isRecording) {
                this.pause();
                return;
            } else { /* keep going */ }
        } else {
    ...
    prepare: async function () {
        ...
        const preparePromise = new Promise((resolve, reject) =>{
            ...
            const onPlayerReady = () => { 
                $(_this).off("playerready", onPlayerReady);
                _this._reset();
                _this._debugLog("on playready");
                resolve();
            };
            $(_this).on("playerready", onPlayerReady);
            _this.play();
